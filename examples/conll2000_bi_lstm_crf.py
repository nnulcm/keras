...
total_emb = concatenate([word_emb, pos_emb], axis=2)

bilstm = Bidirectional(LSTM(lstm_dim, return_sequences=True, dropout=0.2, recurrent_dropout=0.2))(total_emb)
...
model = Model(inputs=[word_input, pos_input], outputs=[crf_output])
...
model.fit([X_words_train, X_pos_train], y_train,
          batch_size=batch_size, epochs=3, callbacks=[conlleval])
